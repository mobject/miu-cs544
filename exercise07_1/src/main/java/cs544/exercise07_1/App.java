...
public class App {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            @SuppressWarnings("unchecked")
            List<Airline> airlines = session.createQuery("select a from Airline a JOIN a.flights f where f.airplane.model = 'A380'").list();
        ...
        try {
            ...
            LocalDateTime localDateTime = LocalDateTime.of(2009,8,7,12,0,0);
            Date date = Date.from(localDateTime.toInstant(ZoneOffset.UTC));
            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("from Flight f where f.departureTime < :departureTime and f.departureDate < :departureDate")
                    .setParameter("departureTime", date, TimestampType.INSTANCE)
                    .setParameter("departureDate", date, DateType.INSTANCE)
//                    .setParameter("departureTime", localDateTime.toLocalTime())
                    .list();
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
